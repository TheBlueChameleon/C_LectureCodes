...
source:
https://github.com/Contagious06/console-snake-game/blob/master/src/snake.c
altered by Stefan Hartinger to compile without warings in gcc, standard C11
*/
...
int collisionSnake (int x, int y, int snakeXY[2][SNAKE_ARRAY_SIZE], int snakeLength, int detect)
{
  int i;
...
void inputScore(int score) //This seriously needs to be cleaned up
{
  ...
  fgets(name, 20, stdin); // Originally: gets(name);
